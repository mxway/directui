...
#else
			snprintf(szBuf, MAX_PATH - 1, _T("file='%s' corner='600,200,1,1'"), param.bgimage.GetData());
#endif
...
#else
		snprintf(szBuf, MAX_PATH - 1, _T("file='%s' corner='600,200,1,1'"), bgimage_.GetData());
#endif
		background->SetBkImage(szBuf);
...
#else
		snprintf(szBuf, MAX_PATH - 1, _T("%s"), friend_.logo.GetData());
#endif
...
void ChatDialog::Notify(TNotifyUI& msg)
{
	...
	else if (msg.sType.CompareNoCase("click") == 0)
	{
		...
		else if (msg.pSender->GetName().Compare(kMinButtonControlName) == 0)
		{
#if defined(UNDER_CE)
		...
		else if (msg.pSender->GetName().CompareNoCase(kFontButtonControlName) == 0)
		{
			CContainerUI* pFontbar = static_cast<CContainerUI*>(m_PaintManager.FindControl(kFontbarControlName));
			if (pFontbar != NULL)
				pFontbar->SetVisible(!pFontbar->IsVisible());
		}
		else if (msg.pSender->GetName().CompareNoCase(kEmotionButtonControlName) == 0)
		{
			POINT pt = {0};
		...
		else if (msg.pSender->GetName().CompareNoCase(kSendButtonControlName) == 0)
        {
            SendMsg();
		}
	}
    else if( msg.sType.CompareNoCase("return") == 0 )
    {
        if (msg.pSender->GetName().Compare(kInputRichEditControlName) == 0)
        {
            SendMsg();
        }
    }
	else if (msg.sType.Compare("timer") == 0)
	{
		return OnTimer(msg);
	}
	else if (msg.sType.CompareNoCase("selectchanged") == 0)
	{
		if (msg.pSender->GetName().CompareNoCase(kColorButtonControlName) == 0)
		{
			CContainerUI* pFontbar = static_cast<CContainerUI*>(m_PaintManager.FindControl(kFontbarControlName));
			if (pFontbar != NULL)
			{
		...
		else if (msg.pSender->GetName().CompareNoCase(kBoldButtonControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kItalicButtonControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(KUnderlineButtonControlName) == 0)
		{
	...
	else if (msg.sType.CompareNoCase("itemselect") == 0)
	{
		if (msg.pSender->GetName().CompareNoCase(kFontTypeControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kFontSizeControlName) == 0)
		{
