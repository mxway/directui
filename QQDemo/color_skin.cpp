...
{
	if (msg.sType.CompareNoCase("click") == 0)
	{
		...
		if (pTabControl != NULL)
		{
			...
			{
				if (strstr(msg.pSender->GetName().GetData(), "colour_") != 0)
				{
	...
	else if (msg.sType.CompareNoCase("valuechanged") == 0)
	{
		CTabLayoutUI* pTabControl = static_cast<CTabLayoutUI*>(m_PaintManager.FindControl(kTabControlName));
		if (pTabControl != NULL)
		{
			if (pTabControl->GetCurSel() == 0)
			{
				...
				if ((AdjustColorSliderR != NULL) && (AdjustColorSliderG != NULL) && (AdjustColorSliderB != NULL))
				{
					if ((msg.pSender->GetName().CompareNoCase(kAdjustColorSliderRControlName) == 0) ||
						(msg.pSender->GetName().CompareNoCase(kAdjustColorSliderGControlName) == 0) ||
						(msg.pSender->GetName().CompareNoCase(kAdjustColorSliderBControlName) == 0))
					{
	...
	{
		CTabLayoutUI* pTabControl = static_cast<CTabLayoutUI*>(m_PaintManager.FindControl(kTabControlName));
		if (msg.pSender->GetName().CompareNoCase(kAdjustColorControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kAdjustBkControlName) == 0)
		{
