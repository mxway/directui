...
Node* CFriendsUI::AddNode(const FriendListItemInfo& item, Node* parent)
{
    ...
    if( m_dlgBuilder.GetMarkup()->Error() ) {
...
#else
			snprintf(szBuf, MAX_PATH - 1, _T("%s"), item.logo.GetData());
#endif
...
#else
		snprintf(szBuf, MAX_PATH - 1, _T("%s"), item.nick_name.GetData());
#endif
		html_text += szBuf;
	}
...
#else
			snprintf(szBuf, MAX_PATH - 1, _T("<x 20><c #808080>%s</c>"), item.description.GetData());
#endif
...
bool CFriendsUI::SelectItem(int iIndex, bool bTakeFocus)
{
	...
	if (!CListUI::SelectItem(iIndex, bTakeFocus))
		return false;
