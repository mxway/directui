...
{
	if (msg.sType.CompareNoCase("windowinit") == 0)
	{
		OnPrepare(msg);
	}
	else if (msg.sType.CompareNoCase("killfocus") == 0)
	{
		if (msg.pSender->GetName().CompareNoCase( kSignatureControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kSearchEditControlName) == 0)
		{
			msg.pSender->SetVisible(false);
	...
	else if (msg.sType.CompareNoCase("click") == 0)
	{
		if (msg.pSender->GetName().CompareNoCase(kCloseButtonControlName) == 0)
		{
			OnExit(msg);
		}
		else if (msg.pSender->GetName().CompareNoCase(kMinButtonControlName) == 0)
		{
#if defined(UNDER_CE)
		...
		else if (msg.pSender->GetName().CompareNoCase(kMaxButtonControlName) == 0)
		{
#if defined(UNDER_CE)
		...
		else if (msg.pSender->GetName().CompareNoCase(kRestoreButtonControlName) == 0)
		{
#if defined(UNDER_CE)
		...
		else if (msg.pSender->GetName().CompareNoCase(kHideLeftMainPannelControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kShowLeftMainPannelControlName) == 0)
		{
			CControlUI* left_main_pannel = m_PaintManager.FindControl(kLeftMainPannelControlName);
			CControlUI* hide_left_main_pannel = m_PaintManager.FindControl(kHideLeftMainPannelControlName);
			CControlUI* show_left_main_pannel = m_PaintManager.FindControl(kShowLeftMainPannelControlName);
		...
		else if (msg.pSender->GetName().CompareNoCase(kSignatureTipsControlName) == 0)
		{
			msg.pSender->SetVisible(false);
		...
		else if (msg.pSender->GetName().CompareNoCase(kSearchEditTipControlName) == 0)
		{
			msg.pSender->SetVisible(false);
		...
		else if (msg.pSender->GetName().CompareNoCase(kChangeBkSkinControlName) == 0)
		{
			CControlUI* background = m_PaintManager.FindControl(kBackgroundControlName);
			if (background != NULL)
			{
				...
				{
					param.bkcolor = background->GetBkColor();
					if (background->GetBkImage().GetLength() > 0)
					{
#if defined(UNDER_WINCE)
		...
		else if (msg.pSender->GetName().CompareNoCase(kChangeColorSkinControlName) == 0)
		{
			CDuiRect rcWindow;
	...
	else if (msg.sType.CompareNoCase("timer") == 0)
	{
		return OnTimer(msg);
	}
	else if (msg.sType.CompareNoCase("selectchanged") == 0)
	{
		CTabLayoutUI* pTabControl = static_cast<CTabLayoutUI*>(m_PaintManager.FindControl(kTabControlName));
		if (msg.pSender->GetName().CompareNoCase(kFriendButtonControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kGroupButtonControlName) == 0)
		{
		...
		else if (msg.pSender->GetName().CompareNoCase(kMicroBlogButtonControlName) == 0)
		{
	...
	else if (msg.sType.CompareNoCase("itemactivate") == 0)
	{
		CTabLayoutUI* pTabControl = static_cast<CTabLayoutUI*>(m_PaintManager.FindControl(kTabControlName));
		if (pTabControl != NULL)
		{
				...
				if ((pFriendsList != NULL) &&  pFriendsList->GetItemIndex(msg.pSender) != -1)
				{
					if (_tcsicmp(msg.pSender->GetClass(), DUI_CTR_LISTCONTAINERELEMENT) == 0)
					{
						...
						if (!pFriendsList->CanExpand(node) && (background != NULL))
						{
							...
							{
								if (citer->id.CompareNoCase(node->data().value.GetData()) == 0)
								{
									friend_info = *citer;
									break;
								}
	...
	else if (msg.sType.CompareNoCase("itemclick") == 0)
	{
		CTabLayoutUI* pTabControl = static_cast<CTabLayoutUI*>(m_PaintManager.FindControl(kTabControlName));
		if (pTabControl != NULL)
		{
