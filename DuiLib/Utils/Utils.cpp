...
namespace DuiLib
{
    ...
    char *Utf8ToUtf16(const char *utf8Str,int maxCharNumber) {
        if(utf8Str==nullptr)
        {
            return nullptr;
        }
        int nLen = ::MultiByteToWideChar(CP_UTF8,0,utf8Str,maxCharNumber,NULL,0);
        wchar_t *unicode = new wchar_t [nLen+1];
        memset(unicode,0,sizeof(wchar_t)*(nLen+1));
        ::MultiByteToWideChar(CP_UTF8,0,utf8Str,maxCharNumber,unicode,nLen);
        return (char*)unicode;
    }

    STRINGorID::STRINGorID(const char *lpString) : m_lpstr(lpString)
    {
    }
    ...
    CDuiPoint::CDuiPoint(const char *pstrValue)
    {
        if (pstrValue == NULL || *pstrValue == '\0') x = y = 0;
        char *pstr = NULL;
        x = y = strtol(pstrValue, &pstr, 10); ASSERT(pstr);
        y = strtol(pstr + 1, &pstr, 10);      ASSERT(pstr);
    }

    CDuiString CDuiPoint::ToString()
    {
        CDuiString sPoint;
        sPoint.SmallFormat("%ld,%ld", x, y);
        return sPoint;
    }
    ...
    CDuiSize::CDuiSize(const char *pstrValue)
    {
        if (pstrValue == NULL || *pstrValue == '\0') cx = cy = 0;
        char *pstr = NULL;
        cx = cy = strtol(pstrValue, &pstr, 10); ASSERT(pstr);
        cy = strtol(pstr + 1, &pstr, 10);       ASSERT(pstr);
    }

    CDuiString CDuiSize::ToString()
    {
        CDuiString sSize;
        sSize.SmallFormat("%ld,%ld", cx, cy);
        return sSize;
    }
    ...
    CDuiRect::CDuiRect(const char *pstrValue)
    {
        if (pstrValue == NULL || *pstrValue == '\0') left = top = right = bottom = 0;
        char *pstr = NULL;
    ...
    {
        CDuiString sRect;
        sRect.SmallFormat("%ld,%ld,%ld,%ld", left, top, right, bottom);
        return sRect;
    }
	...
	CDuiString::CDuiString(const char ch) : m_pstr(m_szBuffer)
	{
		m_szBuffer[0] = ch;
		m_szBuffer[1] = '\0';
	}

	CDuiString::CDuiString(const char *lpsz, int nLen) : m_pstr(m_szBuffer)
	{      
	...
	void CDuiString::Append(const char *pstr)
	{
		...
		if( nNewLength >= MAX_LOCAL_STRING_LEN ) {
			if( m_pstr == m_szBuffer ) {
				...
				memcpy(m_pstr + originStringLen,pstr, strlen(pstr)+1);
			}
			else {
				m_pstr = static_cast<char*>(realloc(m_pstr, (nNewLength + 1) * sizeof(char)));
				memcpy(m_pstr+GetLength(),pstr, strlen(pstr)+1);
			}
		}
		else {
			if( m_pstr != m_szBuffer ) {
			...
			memcpy(m_szBuffer+GetLength(),pstr,strlen(pstr)+1);
		}
	}

	void CDuiString::Assign(const char *pstr, int cchMax)
	{
		if( pstr == NULL ) pstr = "";
	...
	const char *CDuiString::GetData() const
	{
		return m_pstr;
	}

    char CDuiString::GetAt(int nIndex) const
	{
		return m_pstr[nIndex];
	}

    char CDuiString::operator[] (int nIndex) const
	{ 
		return m_pstr[nIndex];
	}   
	...
	const CDuiString& CDuiString::operator=(const char *lpStr)
	{      
		if ( lpStr )
		{
	...
	const CDuiString& CDuiString::operator=(const char ch)
	{
		Empty();
		m_szBuffer[0] = ch;
		m_szBuffer[1] = '\0';
	...
	CDuiString CDuiString::operator+(const char *lpStr) const
	{
		if ( lpStr )
		{
			ASSERT(!::IsBadStringPtr(lpStr,-1));
	...
	bool CDuiString::operator == (const char *str) const { return (Compare(str) == 0); };

    bool CDuiString::operator==(const CDuiString &str) const { return (Compare(str.GetData())==0); }
	...
	void CDuiString::SetAt(int nIndex, char ch)
	{
		ASSERT(nIndex>=0 && nIndex<GetLength());
		m_pstr[nIndex] = ch;
	}

	int CDuiString::Compare(const char * lpsz) const
	{ 
		return strcmp(m_pstr, lpsz);
	}

	int CDuiString::CompareNoCase(const char *lpsz) const
	{ 
	...
	int CDuiString::Find(char ch, int iPos /*= 0*/) const
	{
	...
	int CDuiString::Find(const char *pstrSub, int iPos /*= 0*/) const
	{
	...
	int CDuiString::ReverseFind(char ch) const
	{
	...
	int CDuiString::Replace(const char *pstrFrom, const char *pstrTo)
	{
		CDuiString sTemp;
		int nCount = 0;
		int iPos = Find(pstrFrom);
		if( iPos < 0 ) return 0;
		int cchFrom = strlen(pstrFrom);
		int cchTo = strlen(pstrTo);
		while( iPos >= 0 ) {
			sTemp = Left(iPos);
			sTemp += pstrTo;
	...
	int CDuiString::Format(const char *pstrFormat, ...)
	{
		char *szSprintf = NULL;
		va_list argList;
        int nLen;
	...
	int CDuiString::SmallFormat(const char *pstrFormat, ...)
	{
		//CDuiString sFormat = pstrFormat;
        char szBuffer[64] = { 0 };
		va_list argList;
		va_start(argList, pstrFormat);
		//int iRet = snprintf(szBuffer,64,sFormat,argList);
		int iRet = vsnprintf(szBuffer,64,pstrFormat, argList);
	...
	LPVOID CDuiStringPtrMap::Find(const CDuiString &key, bool optimize) const
	{
	...
	bool CDuiStringPtrMap::Insert(const CDuiString &key, LPVOID pData)
	{
	...
	LPVOID CDuiStringPtrMap::Set(const CDuiString &key, LPVOID pData)
	{
	...
	bool CDuiStringPtrMap::Remove(const CDuiString &key)
	{
	...
	const char *CDuiStringPtrMap::GetAt(int iIndex) const
	{
	...
	const char *CDuiStringPtrMap::operator[] (int nIndex) const
	{
		return GetAt(nIndex);
	}
