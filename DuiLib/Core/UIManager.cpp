...
void CPaintManagerUI::SetResourcePath(const CDuiString &pStrPath)
{
...
const CDuiString & CPaintManagerUI::GetName() const
{
	return m_sName;
}
...
const CDuiString & CPaintManagerUI::GetLayeredImage()
{
...
void CPaintManagerUI::SetDefaultFont(const CDuiString &pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bShared)
{
    LOGFONT lf = { 0 };
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
    _tcsncpy(lf.lfFaceName, pStrFontName.GetData(), LF_FACESIZE);
    lf.lfCharSet = DEFAULT_CHARSET;
    lf.lfHeight = -nSize;
...
HFONT CPaintManagerUI::AddFont(int id, const CDuiString &pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bShared)
{
    LOGFONT lf = { 0 };
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
    _tcsncpy(lf.lfFaceName, pStrFontName.GetData(), LF_FACESIZE);
...
HFONT CPaintManagerUI::GetFont(const CDuiString &pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
...
const TImageInfo* CPaintManagerUI::GetImage(const CDuiString &bitmap)
{
...
const TImageInfo* CPaintManagerUI::GetImageEx(const CDuiString &bitmap, const CDuiString &type, DWORD mask, bool bUseHSL)
{
...
const TImageInfo* CPaintManagerUI::AddImage(const CDuiString &bitmap, const CDuiString &type, DWORD mask, bool bUseHSL, bool bShared)
{
	if( bitmap.IsEmpty() || bitmap[0] == '\0' ) return NULL;

    TImageInfo* data = NULL;
    if( !type.IsEmpty() ) {
        if( isdigit(bitmap.GetAt(0)) ) {
            LPTSTR pstr = NULL;
            int iIndex = strtol(bitmap.GetData(), &pstr, 10);
            data = CRenderEngine::LoadImage(iIndex, type.GetData(), mask);
        }
        else {
            data = CRenderEngine::LoadImage(bitmap.GetData(), type.GetData(), mask);
        }
    }
    else {
        data = CRenderEngine::LoadImage(bitmap.GetData(), NULL, mask);
    }

	if( data == NULL ) return NULL;
	data->bUseHSL = bUseHSL;
	if( !type.IsEmpty() ) data->sResType = type;
	data->dwMask = mask;
	if( data->bUseHSL ) {
...
CControlUI* CPaintManagerUI::FindControl(const CDuiString &pstrName) const
{
    ASSERT(m_pRoot);
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromName(CControlUI* pThis, LPVOID pData)
{
    ...
    return (strcasecmp(sName.GetData(),pstrName)==0)?pThis:NULL;
}
