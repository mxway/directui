                                   ...
                                   CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    CDuiString  xmlPath = CPaintManagerUI::GetResourcePath() + xml.m_lpstr;
    m_xml.LoadFile(xmlPath.GetData());
...
{
    m_pCallback = pCallback;
    tinyxml2::XMLElement *root = m_xml.RootElement();
    ...
    if( pManager ) {
        ...
        for( tinyxml2::XMLElement *node = root->FirstChildElement() ; node != nullptr; node = node->NextSiblingElement() ) {
            ...
            if( _tcsicmp(pstrClass, _T("Image")) == 0 ) {
				...
				int id = -1;
				const char *pMultiLanguage = NULL;
                for(auto attribute=node->FirstAttribute(); attribute!=nullptr; attribute = attribute->Next()) {
					pstrName = attribute->Name();
					pstrValue = attribute->Value();
					if( _tcsicmp(pstrName, _T("id")) == 0 ) {
        ...
        if( _tcsicmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for(auto attribute=root->FirstAttribute(); attribute!=nullptr; attribute = attribute->Next()) {
                    pstrName = attribute->Name();
                    pstrValue = attribute->Value();
...
tinyxml2::XMLDocument * CDialogBuilder::GetMarkup()
{
    return &m_xml;
}
...
{
    snprintf(pstrMessage,cchMax,"%s",m_xml.ErrorStr());
}

void CDialogBuilder::GetLastErrorLocation(LPTSTR pstrSource, SIZE_T cchMax) const
{
    snprintf(pstrSource,cchMax,"ErrorLine:%d",m_xml.ErrorLineNum());
}

CControlUI* CDialogBuilder::_Parse(tinyxml2::XMLElement* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    IContainerUI* pContainer = NULL;
    CControlUI* pReturn = NULL;
    for( tinyxml2::XMLElement *node = pRoot->FirstChildElement() ; node != nullptr; node = node->NextSiblingElement()) {
        ...
        if( _tcsicmp(pstrClass, _T("Include")) == 0 ) {
            ...
            int count = 1;
            LPTSTR pstr = NULL;
            const char *szValue = node->Attribute("count");
            if(szValue)
            {
                count = strtol(szValue,&pstr,10);
            }
            szValue = node->Attribute("source");
            if(!szValue)
            {
                continue;
            }
			...
			for ( int i = 0; i < count; i++ ) {
			    pControl = builder.Create(szValue,0,m_pCallback,pManager,pParent);
		...
		//树控件XML解析
			...
			if( node->HasAttributes() ) {
                ...
                for(auto attribute=node->FirstAttribute(); attribute!=nullptr; attribute = attribute->Next()) {
					pNode->SetAttribute(attribute->Name(), attribute->Value());
				}
			}
		...
		if( pParent != NULL ) {
            ...
            const char *szValue = node->Attribute("cover");
            if( szValue != nullptr && _tcscmp(szValue, _T("true")) == 0 ) {
