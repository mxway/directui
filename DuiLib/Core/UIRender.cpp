					  ...
					  TDrawInfo& drawInfo)
{
	...
	if( drawInfo.pImageInfo == NULL ) {
		...
		if( bUseRes == false ) {
			data = pManager->GetImageEx(sImageName, "", dwMask, bUseHSL);
		}
		else {
			data = pManager->GetImageEx(sImageName, sImageResType, dwMask, bUseHSL);
		}
...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, const CDuiString &pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    if( pstrText.IsEmpty() || pManager == NULL ) return;

	CDuiString sText = pstrText;
	CPaintManagerUI::ProcessMultiLanguageTokens(sText);
	wchar_t   *unicodeText = (wchar_t*)Utf8ToUtf16(sText.GetData());
	//pstrText = sText;

    ::SetBkMode(hDC, TRANSPARENT);
    ::SetTextColor(hDC, RGB(GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
    HFONT hOldFont = (HFONT)::SelectObject(hDC, pManager->GetFont(iFont));
    ::DrawTextW(hDC,unicodeText, -1, &rc, uStyle | DT_NOPREFIX);
    delete []unicodeText;
    ::SelectObject(hDC, hOldFont);
}

void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, const CDuiString &strText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, int iDefaultFont, UINT uStyle)
{
    // ���ǵ���xml�༭����ʹ��<>���Ų����㣬����ʹ��{}���Ŵ���
    ...
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    if( strText.IsEmpty() || pManager == NULL ) return;
	...
	if( ((uStyle & DT_CENTER) != 0 || (uStyle & DT_RIGHT) != 0 || (uStyle & DT_VCENTER) != 0 || (uStyle & DT_BOTTOM) != 0) && (uStyle & DT_CALCRECT) == 0 ) {
		...
		int nLinks = 0;
		DrawHtmlText(hDC, pManager, rcText, strText, dwTextColor, NULL, NULL, nLinks, iDefaultFont, uStyle | DT_CALCRECT & ~DT_CENTER & ~DT_RIGHT & ~DT_VCENTER & ~DT_BOTTOM);
		if( (uStyle & DT_SINGLELINE) != 0 ){
    ...
    const char *pstrLineBegin = strText.GetData();
    ...
    const char *pstrText = strText.GetData();
    while( *pstrText != _T('\0') ) {
...
SIZE CRenderEngine::GetTextSize( HDC hDC, CPaintManagerUI* pManager , LPCTSTR pstrText, int iFont, UINT uStyle )
{
	...
	::SetBkMode(hDC, TRANSPARENT);
	HFONT hOldFont = (HFONT)::SelectObject(hDC, pManager->GetFont(iFont));
	GetTextExtentPoint32(hDC, sText.GetData(), sText.GetLength() , &size);
	::SelectObject(hDC, hOldFont);
	return size;
}
